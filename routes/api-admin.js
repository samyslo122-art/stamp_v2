...
router.get(
    "/api/admin/groups/lookup/:qrCode",
    requireAdmin,
    async (req, res) => {
        try {
...
router.post("/api/admin/stamps/issue", requireAdmin, async (req, res) => {
    try {
        ...
        if (existing.rows.length > 0) {
            return res
                .status(409)
                .json({ error: "Already stamped this booth this round" });
        }
...
router.post("/api/admin/redemption/lookup", requireAdmin, async (req, res) => {
    console.log("[API] /api/admin/redemption/lookup called", {
        body: req.body,
    ...
    try {
        ...
        const categoriesCollected = [
            ...new Set(stamps.map((s) => s.booth_category)),
        ];
        ...
        const tiers = await checkRedemptionEligibility(
            stampTotal,
            categoriesCollected,
            redeemedTiers,
        );
...
router.post("/api/admin/redemption/claim", requireAdmin, async (req, res) => {
    console.log("[API] /api/admin/redemption/claim called", {
        body: req.body,
        session: req.session?.isAdmin,
    });
    try {
        ...
        // SSE push
        sse.sendEvent(player.unique_id, "student:update", {
            type: "redemption",
            tierKey,
            giftName,
        });
